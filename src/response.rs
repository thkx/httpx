...
impl HttpResponse {
    ...
    pub fn html(&mut self, body: &str, status: HttpStateCode) -> &mut Self {
        self.insert_header("Content-Type", "text/html");
        self.set_http_state_code(status);
        self.write_str(body);
        self
    }

    pub fn json(&mut self, body: &str, status: HttpStateCode) -> &mut Self {
